...
+ ERA5-Land (Apr-Oct 2022 aggregates)
...
// 4) ERA5-Land Daily Aggregated (Apr–Oct 2022)
...
var t2m_c = era5.select('temperature_2m').mean().subtract(273.15).rename('t2m_C');
var tp_sum = era5.select('total_precipitation_sum').sum().rename('tp_sum');
var sm1 = era5.select('volumetric_soil_water_layer_1').mean().rename('sm1');
var sm2 = era5.select('volumetric_soil_water_layer_2').mean().rename('sm2');

var era5_stack = ee.Image.cat([t2m_c, tp_sum, sm1, sm2]).clip(region);
print('ERA5 bands:', era5_stack.bandNames());

// 5) Combine predictors (S2 + S1 + ERA5)
var predictors = s2_stack.addBands(s1_stack).addBands(era5_stack);
...
Export.table.toDrive({
  collection: training,
  description: 'DE_LUCAS2018_S1S2_ERA5_2022_training',
  fileFormat: 'CSV'
});
